...
import { auth } from '../../../../../src/store/types'
...
export default {
  ...
  submitAction: function (fileList) {
    if (fileList.length > 0) {
      ...
      if (process.env.NODE_ENV === 'production') {
        imageUploadUrl = 'http://api.onesell.co.kr:16061/file-upload';
      } else {
        imageUploadUrl = 'http://dv.api.onesell.co.kr:16061/file-upload';
      }

      const imageType = this.context.option.imageType;
      ...
      if (typeof imageUploadUrl === 'string' && imageUploadUrl.length > 0) {
        ...
        this.context.image._xmlHttp.setRequestHeader('Authorization', 'Bearer ' + store.getters[auth.getters.token]);
  ...
  callBack_imgUpload: function (linkValue, linkNewWindow, width, align, update, updateElement) {
    if (this.context.image._xmlHttp.readyState === 4) {
      ...
      if (this.context.image._xmlHttp.status === 200 && response.error === undefined ) {
        if (response.status === 'ERROR' && response.message) {
          this.closeLoading();
          if (this._imageUploadError(response.message, response.result)) {
            notice.open.call(this, response.message);
          }
        } else {
          const fileList = []
